...
Guidance for AI coding agents (Claude Code, Codex, Gemini CLI, …) working in this repository.
Claude Code loads this file via the `@AGENTS.md` import in `CLAUDE.md` — edit this file, not that one.
...
- Active nav link uses `var(--sfsc-blue-light)` (`#70709D`) background with white text (4.67:1). Do not use orange with white text — that's only 2.72:1 (WCAG fail). Orange-with-dark-text is reserved for the guide side nav's active tab (CMS setting).
